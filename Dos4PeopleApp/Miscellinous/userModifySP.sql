USE [DOSDB]
GO
/****** Object:  StoredProcedure [dbo].[UserPackageRequest_Add]    Script Date: 4/28/2022 5:48:15 PM ******/
SET ANSI_NULLS ON
GO
...
------------------------------------------------------------------------------- */
CREATE PROCEDURE [dbo].[UserInfo_Modify]
	@UserId	varchar(37),	
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM tblUser WHERE Email = @Email AND UserId <> @UserId)  
			GOTO ERR_EMAIL_EXISTS;

	 -- ASSERT (If Mobile Already Exists)
	IF NOT EXISTS (SELECT * FROM tblUser WHERE Mobile = @Mobile AND UserId <> @UserId)  
